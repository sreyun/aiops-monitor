...
- 正式发布线：`v0.16.0` → `v0.16.8` → `v0.17.0` → `v0.18.0` → `v0.18.9` → `v0.19.0` → `v0.19.47` → `v0.19.48` → `v0.19.49` → `v0.19.50` → `v0.19.51` → `v0.19.52` → `v0.19.53` → `v0.19.54` → `v0.19.55` → `v0.19.56` → `v0.19.57` → **`v0.19.58`**
...
- **主机树/系统树自动刷新**：服务端推送 `hosts_changed`；经典版与 Vue 新壳在主机加入/离开时自动刷新主机树、系统树及剧本/巡检/安全/硬件/容器等选机树，无需再点手动刷新；合并去重避免双拉与重建风暴。
- **Web v2 Phase 2**：远程桌面（JPEG/质量/文件传输/回放）、自动化（YAML 步骤 + 执行日志 + 体检）、终端搜索/历史/文件传输；Vue 路线图加深与 `docs/v2-migration.md`。
...
- **Web v2 完整性修复**：登录后正确进入新壳；修复 `.gitignore` 误忽略 `frontend/.../security`；补齐导航桥接与经典版深链。
  - 登录：`setAuth("yes")` + `/me` 确认，避免卡在登录页。
  - 经典版：`/?ui=legacy#view` 首屏/hashchange 全量视图路由；支持 `#dashboard/{id}`、`#settings`、`#sql-toolkit`、`#security-overview` 等。
  - 新壳导航：补齐日志/资源/网络/自动化/转发/数据源桥接页；安全中心改为 KPI 摘要而非 JSON dump。
  - 测试：Vitest（display/normalize）；Go `web_v2_test` 覆盖 embed 与 `/v2` 公开路径。
...
- **Web 控制台 v2（绞杀者起步）**：新增 `frontend/`（React 19 + TypeScript + Vite），构建产物嵌入 `cmd/server/web/v2`；默认 `/` 进入新壳，`/?ui=legacy` 回退经典版；支持 Nginx 独立部署（见 `frontend/deploy/nginx.v2.example.conf`）。
  - **P0**：工程基线、`make web`、Go embed/`/v2` 静态路由、Hash 路由壳、登录与布局。
  - **P1**：API client、TanStack Query、`/ws/push` 失效、HostLabel 脱敏展示、i18n/设计令牌、ECharts 懒加载封装。
  - **P2**：概览 / 告警（ack·静默）/ 主机树与列表 / 事件日志 / 看板列表 / 拨测列表迁入 React（路由级 code-split）。
  - **P3**：安全 / SRE / 设置提供新壳入口与经典版深度桥接；经典版保留终端·桌面·看板编辑·剧本 AI 等重交互直至后续批次清退。
  - **体验**：KPI 可点击跳转、列表骨架屏、主题切换、连接态脉冲、推送优先轮询降频。
