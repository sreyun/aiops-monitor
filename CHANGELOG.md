...
- **MCP Server 工具治理**：新增 `list_hosts`（分页/搜索/在线过滤）；`query_containers` 默认全平台摘要、按 `host_id` 查明细并支持 limit/offset/status；禁止 12KB 硬切半截 JSON，改为 48KB 软上限 + 结构化截断提示；`query_k8s` events/log 同步治理；作用域与值班/对话提示引导正确工具链。
...
- **仓库治理**：收紧 `.gitignore`，过滤运行时产物与敏感文件。
