...
| **AI 运维助手** | Hermes 自主 Agent（Function Calling）+ AI 巡检诊断 + RAG 相似案例检索（可插拔 LLM，零依赖启发式兜底） |
...
- **Hermes 自主 Agent**：面板「AI 助手」多轮对话（SSE 流式 + 会话持久化），支持 **Function Calling 工具调用**——查询指标 / 检索日志 / 列出告警 / 检索相似案例 / 只读终端巡检；可配置 Hermes 规则（rules / templates）与自动审批、终端只读开关。
- 配置：仅在「AI 配置」填写 `ai{enabled, endpoint, api_key, model, hermes_enabled, hermes_auto_approve, hermes_terminal_enabled}` 后启用智能分析；密钥经 `AIOPS_SECRET_KEY` AES 加密落库。
...
│   │   ├── hermes.go               # Hermes 自主运维 Agent 框架
│   │   ├── hermes_suggest.go       # Hermes 对话提示建议
│   │   ├── message.go              # 统一消息中心
...
| GET | `/api/v1/ai/inspections` | 巡检报告列表 |
| GET | `/api/v1/hermes/chat` | Hermes 自主 Agent 对话 |
| GET | `/api/v1/hermes/suggestions` | 对话快捷问句 |
| GET/POST/DELETE | `/api/v1/hermes/sessions[/{id}[/undo]]` | Hermes 会话管理 |
| GET/POST/DELETE | `/api/v1/hermes/rules` · `/templates` | Hermes 规则 / 模板 |
| **消息中心** | | |
...
- [x] AI 运维助手：可插拔 LLM 巡检诊断 + RAG 相似案例 + Hermes 自主 Agent（Function Calling）
