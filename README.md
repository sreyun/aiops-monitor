...
| **终端增强** | 会话录制与回放、多标签终端、只读旁观模式、命令级审计；Windows ConPTY **chcp 65001** + GBK→UTF-8 转换保障跨平台中文不乱码 |
...
- [x] **自动化运维**：剧本编排（多步骤、按分类/主机选择目标）、批量并行执行（实时状态 + 输出展示、超时控制、失败继续选项）、执行历史与报告；sentinel 命令完成检测 + 输出清理（ANSI/提示符/命令回显）
