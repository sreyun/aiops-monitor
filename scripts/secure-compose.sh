#!/usr/bin/env bash
#
# secure-compose.sh — 下载 docker-compose.yml 并自动注入随机密钥
# =========================================================================
# 适用环境：Linux / macOS（依赖 bash、curl、tr、head、sed，均为系统自带）
#
# 作用：
#   1. 下载 docker-compose.yml 到当前目录
#   2. 生成 PostgreSQL 密码（20 位纯字母数字，A-Za-z0-9）
#   3. 生成 AIOPS_SECRET_KEY（aiops- 前缀 + 44 位随机字母数字，总长度 50 位）
#   4. 使用 sed 替换占位符，同步更新 AIOPS_POSTGRES_DSN 中的密码
#
...
#
# 可用环境变量覆盖：
#   COMPOSE_URL  编排文件地址（默认仓库 master 分支）
...
# ---------- 2. 生成随机密钥（与 README 内联命令完全一致） ----------
# PostgreSQL 密码：20 位纯字母数字（A-Za-z0-9）
PG_PWD=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c20)

# AIOPS_SECRET_KEY：aiops- 前缀 + 44 位随机字母数字，总长度 50 位
SECRET_KEY="aiops-$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c44)"

# ---------- 3. 替换占位符 ----------
echo "==> 正在将随机密钥写入 $OUT_FILE（无需手动修改）"

# 替换 PostgreSQL 密码占位符（两处：AIOPS_POSTGRES_DSN 和 POSTGRES_PASSWORD）
sed -i "s|h3Y7Vmb1CZBOApZM86D|${PG_PWD}|g" "$OUT_FILE"

# 替换 AIOPS_SECRET_KEY 占位符
sed -i "s|aiops-K7p2mQ9vR4xN8wZ3bY6dF1hJ5sL0tGc-CHANGE-ME-2026|${SECRET_KEY}|" "$OUT_FILE"

# ---------- 4. 输出生成的密钥 ----------
echo "PG password: ${PG_PWD}"
...
echo "  浏览器打开 http://localhost:8529  （默认账号 admin / admin，首次登录强制修改密码）"#!/usr/bin/env bash
#
