...
## 更新摘要
**变更内容**   
- **v6.4.0重大界面重写**：前端硬件监控面板完全重构，支持卡片/列表视图自由切换，新增 KPI 摘要条显示整体健康状态、异常部件数、最高温度、总功耗等关键指标，增强详情弹窗展示历史曲线和完整硬件信息
- 实现了交互式展开/收起机制，点击硬件卡片可查看详细数据并自动加载历史趋势图
- 新增了智能排序功能，按健康状态（严重 > 警告 > 正常）优先显示问题设备
- 完善了国际化支持，所有UI文本均支持中/英/繁中三语切换
- 优化了性能，通过DOM操作而非重新渲染实现视图切换，减少不必要的DOM重建
...
**最新更新**：v6.4.0 对硬件监控界面进行了完全重写，新增了卡片/列表视图自由切换、KPI 摘要条显示整体健康状态、异常部件数、最高温度、总功耗等信息，增强了详情弹窗展示历史曲线和完整硬件信息；v6.3.0 修复了重复代码bug导致的硬件卡片点击功能失效问题，新增了内存和DIMM监控、完整传感器数据显示、交互式展开/收起机制、增强的表格结构和国际化支持等核心功能改进；v6.2.6 修复了跨模块主机列表缓存初始化问题，确保硬件监控面板能正确获取主机数据；v6.2.5 增强了硬件报告失败的诊断日志，提供更详细的错误信息和成功状态跟踪。
...
- [cmd/agent/collector_netflow.go:203-263](file://cmd/agent/collector_netflow.go#L203-263)
- [cmd/agent/collector_packet.go:59-113](file://cmd/agent/collector_packet.go#L59-113)
...
- Server 侧
  ...
  - 告警合并：活跃告警 + 最近恢复的历史记录
- 前端
  - **v6.4.0重大重写**：硬件健康面板支持卡片/列表视图自由切换，KPI 摘要条显示整体健康状态、异常部件数、最高温度、总功耗等信息，详情弹窗展示历史曲线和完整硬件信息
  - **v6.3.0增强**：交互式展开/收起机制，显示CPU/温度/风扇/电源/固件/内存/DIMM等完整信息，增强的表格结构和国际化支持
  - 流量面板：Top Talkers 排行、Flow 明细、筛选与导出
  - 跨模块缓存：通过 `window._cachedHosts` 实现主机列表共享
  - **完整国际化支持**：中/英/繁中三语切换，所有UI文本均支持本地化

**更新**：v6.4.0 对硬件监控界面进行了完全重写，新增了卡片/列表视图自由切换、KPI 摘要条显示整体健康状态、异常部件数、最高温度、总功耗等信息，增强了详情弹窗展示历史曲线和完整硬件信息；v6.3.0 修复了重复代码bug导致的点击功能失效问题，新增了内存和DIMM监控、完整传感器数据显示、交互式展开/收起机制、增强的表格结构和国际化支持；v6.2.6 修复了跨模块缓存初始化问题，确保硬件监控和 NetFlow 面板能正确访问主机列表数据。
...
**更新**：v6.4.0 前端架构完全重写，新增了卡片/列表视图切换、KPI 摘要条、交互式详情弹窗等功能；v6.2.6 增加了跨模块主机列表缓存机制，避免重复请求提升性能。
...
Note over UI,Cache : v6.2.6 新增跨模块缓存
Note over UI : v6.4.0 完全重写的硬件面板
UI->>UI : renderHosts() 调用
UI->>Cache : 设置 window._cachedHosts = hosts
UI->>HardwarePanel : 渲染硬件面板v6.4.0重写
...
**更新**：v6.4.0 对硬件监控界面进行了完全重写，新增了卡片/列表视图自由切换、KPI 摘要条显示整体健康状态、异常部件数、最高温度、总功耗等信息，增强了详情弹窗展示历史曲线和完整硬件信息；v6.3.0 修复了重复代码bug导致的点击功能失效问题，新增了内存和DIMM监控、完整传感器数据显示、交互式展开/收起机制、增强的表格结构和国际化支持；v6.2.6 修复了跨模块缓存初始化问题，确保硬件监控面板能正确获取主机列表数据。
...
participant S as "Server"
Note over HostsJS,HWJS : v6.2.6 跨模块缓存机制
Note over HWJS,I18N : v6.4.0 完全重写的硬件面板
UI->>HostsJS : 渲染主机列表
HostsJS->>HostsJS : renderHosts(hosts)
HostsJS->>HostsJS : window._cachedHosts = hosts
UI->>HWJS : 渲染硬件面板v6.4.0重写
...
章节来源
- [cmd/server/web/js/hardware.js:1-314](file://cmd/server/web/js/hardware.js#L1-L314)
...
- 前端
  ...
  - **v6.4.0性能优化**：硬件面板完全重写，通过DOM操作而非重新渲染实现视图切换，减少不必要的DOM重建；智能排序算法优化大数据量下的渲染性能

**更新**：v6.4.0 对硬件面板进行了完全重写，新增了卡片/列表视图切换、KPI 摘要条等功能，同时优化了性能，通过DOM操作而非重新渲染实现视图切换，减少了不必要的DOM重建；v6.2.6 的跨模块缓存机制显著减少了主机列表的重复请求，提升了硬件监控和 NetFlow 面板的加载性能。
...
**更新**：新增了 v6.4.0 完全重写界面的故障排查步骤，包括卡片/列表视图切换、KPI 摘要条、详情弹窗历史曲线等新功能的排查方法；同时保留了之前版本的故障排查指南。
...
**版本演进亮点**：
- **v6.4.0完全重写**：硬件监控界面完全重构，支持卡片/列表视图自由切换，新增 KPI 摘要条显示整体健康状态、异常部件数、最高温度、总功耗等信息，增强详情弹窗展示历史曲线和完整硬件信息
- **v6.3.0重大增强**：修复了重复代码bug导致的硬件卡片点击功能失效问题，新增了内存和DIMM监控、完整传感器数据显示、交互式展开/收起机制、增强的表格结构和国际化支持等核心功能改进
- v6.2.6 通过跨模块缓存机制解决了硬件监控数据显示问题，提升了前端性能
