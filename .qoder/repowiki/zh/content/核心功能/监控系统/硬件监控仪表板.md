...
## 更新摘要
**变更内容**   
- **新增完整硬件资产导出功能**：支持Excel(.xlsx)、Word(.docx)、Markdown(.md)、PDF四种格式导出，采用零依赖设计，无需第三方库
- **统一文档模型接口**：通过中性文档模型结构体支持多种输出格式，任何面板都能复用
- **国际化支持**：所有导出相关UI文本均支持中/英/繁中三语切换
- **用户界面增强**：在硬件详情弹窗中集成下拉菜单操作，提供便捷的导出入口
- **浏览器原生实现**：Excel/Word使用手写OOXML+最小ZIP打包器，PDF通过浏览器打印实现中文正确显示
...
- 服务端持久化、时序写入与前端可视化
- **新增硬件资产导出功能**：支持多格式导出，满足资产管理需求

系统采用 Go 单二进制服务端 + Agent 上报模式，统一存储为 PostgreSQL（关系数据）+ VictoriaMetrics（时序数据），前端通过内嵌静态资源提供交互式仪表板。

**最新更新**：v6.5.0 对硬件监控界面进行了完全重写，新增了卡片/列表视图自由切换、KPI 摘要条显示整体健康状态、异常部件数、最高温度、总功耗等信息，增强了详情弹窗展示历史曲线和完整硬件信息；**v6.6.0新增完整硬件资产导出功能**，支持Excel/Word/Markdown/PDF四种格式导出，采用零依赖设计，集成到硬件详情弹窗的下拉菜单操作中；v6.4.0 修复了重复代码bug导致的硬件卡片点击功能失效问题，新增了内存和DIMM监控、完整传感器数据显示、交互式展开/收起机制、增强的表格结构和国际化支持等核心功能改进；v6.3.0 修复了跨模块主机列表缓存初始化问题，确保硬件监控面板能正确获取主机数据；v6.2.5 增强了硬件报告失败的诊断日志，提供更详细的错误信息和成功状态跟踪。
...
- [cmd/server/handlers.go:290-298](file://cmd/server/handlers.go#L290-298)
...
- Server 侧
  ...
  - 告警合并：活跃告警 + 最近恢复的历史记录
- 前端
  - **v6.5.0重大重写**：硬件健康面板支持卡片/列表视图自由切换，KPI 摘要条显示整体健康状态、异常部件数、最高温度、总功耗等信息，详情弹窗展示历史曲线和完整硬件信息
  - **v6.6.0新增导出功能**：通用文档导出引擎支持Excel/Word/Markdown/PDF四种格式，集成到硬件详情弹窗下拉菜单
  - **v6.4.0增强**：交互式展开/收起机制，显示CPU/温度/风扇/电源/固件/内存/DIMM等完整信息，增强的表格结构和国际化支持
  - 流量面板：Top Talkers 排行、Flow 明细、筛选与导出
  - 跨模块缓存：通过 `window._cachedHosts` 实现主机列表共享
  - **完整国际化支持**：中/英/繁中三语切换，所有UI文本均支持本地化

**更新**：v6.6.0 新增完整硬件资产导出功能，支持Excel/Word/Markdown/PDF四种格式导出，采用零依赖设计，通过统一的文档模型接口支持多种输出格式，并集成了硬件详情弹窗的下拉菜单操作；v6.5.0 对硬件监控界面进行了完全重写，新增了卡片/列表视图自由切换、KPI 摘要条显示整体健康状态、异常部件数、最高温度、总功耗等信息，增强了详情弹窗展示历史曲线和完整硬件信息；v6.4.0 修复了重复代码bug导致的点击功能失效问题，新增了内存和DIMM监控、完整传感器数据显示、交互式展开/收起机制、增强的表格结构和国际化支持；v6.3.0 修复了跨模块缓存初始化问题，确保硬件监控和 NetFlow 面板能正确访问主机列表数据。
...
- **新增导出流程**：前端构建中性文档模型 → 调用导出引擎 → 生成多格式文件

**更新**：v6.6.0 新增硬件资产导出功能，通过前端导出引擎直接生成Excel/Word/Markdown/PDF文件，无需后端参与；v6.5.0 前端架构完全重写，新增了卡片/列表视图切换、KPI 摘要条、交互式详情弹窗等功能；v6.4.0 增加了跨模块主机列表缓存机制，避免重复请求提升性能。
...
Note over UI,Cache : v6.4.0 新增跨模块缓存
Note over UI : v6.5.0 完全重写的硬件面板
Note over UI,Export : v6.6.0 新增导出功能
UI->>UI : renderHosts() 调用
UI->>Cache : 设置 window._cachedHosts = hosts
UI->>HardwarePanel : 渲染硬件面板v6.5.0重写
HardwarePanel->>Cache : 读取 window._cachedHosts
...
UI->>Export : 构建文档模型并导出
Export->>Export : 生成Excel/Word/Markdown/PDF
Export-->>UI : 下载文件
UI->>Server : GET /api/v1/netflow/summary|flows
Server->>VM/PG : 聚合/明细查询
Server-->>UI : Top-N/明细表
```
...
- **v6.6.0新增导出功能**
  - **通用文档导出引擎**：支持Excel(.xlsx)、Word(.docx)、Markdown(.md)、PDF四种格式
  - **零依赖设计**：Excel/Word使用手写OOXML+最小ZIP打包器，无需第三方库
  - **统一文档模型**：通过中性文档模型结构体支持多种输出格式
  - **下拉菜单集成**：在硬件详情弹窗中集成导出按钮和下拉菜单
  - **国际化支持**：所有导出相关UI文本均支持多语言
  - **浏览器原生实现**：PDF通过浏览器打印实现，确保中文正确显示
- **网络流量面板**
...
**更新**：v6.6.0 新增完整硬件资产导出功能，支持Excel/Word/Markdown/PDF四种格式导出，采用零依赖设计，通过统一的文档模型接口支持多种输出格式，并集成了硬件详情弹窗的下拉菜单操作；v6.5.0 对硬件监控界面进行了完全重写，新增了卡片/列表视图自由切换、KPI 摘要条显示整体健康状态、异常部件数、最高温度、总功耗等信息，增强了详情弹窗展示历史曲线和完整硬件信息；v6.4.0 修复了重复代码bug导致的点击功能失效问题，新增了内存和DIMM监控、完整传感器数据显示、交互式展开/收起机制、增强的表格结构和国际化支持；v6.3.0 修复了跨模块缓存初始化问题，确保硬件监控面板能正确获取主机列表数据。
...
participant ExportJS as "export.js"
...
Note over HWJS,ExportJS : v6.6.0 新增导出功能
UI->>HostsJS : 渲染主机列表
...
UI->>UI : 点击导出按钮v6.6.0新增
UI->>HWJS : 构建文档模型
HWJS->>ExportJS : 调用导出引擎
ExportJS->>ExportJS : 生成Excel/Word/Markdown/PDF
ExportJS-->>UI : 下载文件
UI->>NFJS : 渲染流量面板
NFJS->>HostsJS : 读取 window._cachedHosts
NFJS->>I18N : 获取国际化文本
NFJS->>S : GET /api/v1/netflow/summary?host=&range=&dimension=src_ip&top=10
...
章节来源
- [cmd/server/web/js/hardware.js:1-761](file://cmd/server/web/js/hardware.js#L1-L761)
- [cmd/server/web/js/netflow.js:1-208](file://cmd/server/web/js/netflow.js#L1-L208)
- [cmd/server/web/js/hosts.js:125-129](file://cmd/server/web/js/hosts.js#L125-L129)
- [cmd/server/web/i18n-dashboard.js:436-472](file://cmd/server/web/i18n-dashboard.js#L436-L472)
- [cmd/server/web/js/export.js:1-415](file://cmd/server/web/js/export.js#L1-L415)
...
**更新**：v6.6.0 新增的导出功能在前端直接生成文件，避免了额外的网络请求，提升了用户体验；v6.5.0 对硬件面板进行了完全重写，新增了卡片/列表视图切换、KPI 摘要条等功能，同时优化了性能，通过DOM操作而非重新渲染实现视图切换，减少了不必要的DOM重建；v6.4.0 的跨模块缓存机制显著减少了主机列表的重复请求，提升了硬件监控和 NetFlow 面板的加载性能。
...
**更新**：新增了 v6.6.0 导出功能的故障排查步骤，包括导出菜单交互、文档模型构建、文件格式生成、PDF打印拦截等问题排查方法；同时保留了之前版本的故障排查指南。
...
**版本演进亮点**：
- **v6.6.0新增导出功能**：完整硬件资产导出功能，支持Excel/Word/Markdown/PDF四种格式，采用零依赖设计，集成到硬件详情弹窗下拉菜单
