...
v5.5.0부터 통합 스토리지는 **PostgreSQL(모든 관계형 데이터) + VictoriaMetrics(모든 시계열 데이터)** 로 통일되었으며, 내장된 `aiops.db`는 폐기되었습니다. 새로 구성 키의 AES-256-GCM 저장 암호화, 선택적 TLS 전송 암호화, 최초 로그인 시 강제 보안 초기화, 크로스 플랫폼 부팅 자동 시작 및 프로세스 유지(keepalive)가 추가되었습니다.
