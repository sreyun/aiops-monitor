...
const aiopsBuiltinMetricsHint = "【本平台内置主机指标（优先使用）】\n" +
	...
	"标签：instance=主机名，host=主机ID，可选 category。示例：avg(aiops_cpu_percent)、topk(10, aiops_mem_percent)、" +
	"aiops_cpu_percent{instance=~\"$instance\"}（仅下钻面板，务必 =~）、label_values(aiops_cpu_percent, instance)。"
...
func healAIDashExpr(expr string) string {
	...
	// instance="$instance" / host="$host" → =~ ，兼容「全部」变成 .*
	eqVar := regexp.MustCompile(`\b(instance|host|job|category)\s*=\s*"(\$(?:\{)?(?:instance|host|job|category)(?:\})?)"`)
	out = eqVar.ReplaceAllString(out, `${1}=~"${2}"`)
	return out
}
