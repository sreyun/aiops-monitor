...
if (-not $Probe.Ok) {
  ...
  if ($Blocked) {
    ...
    # Diagnose which control stack is active so the operator picks the right fix.
    $SacOn = $false
    try {
    ...
    if ($SacOn) {
      Write-Host "[AIOps] Detected: Smart App Control is ON (often cannot be turned Off without OS reset)." -ForegroundColor Yellow
      Write-Host "[AIOps] 检测到：智能应用控制已开启（开启后多数无法直接关闭，需重装系统或改用企业 WDAC 放行）。" -ForegroundColor Yellow
    }
    if ($AppLockerOn) {
      Write-Host "[AIOps] Detected: AppLocker effective policy present." -ForegroundColor Yellow
      Write-Host "[AIOps] 检测到：本机存在生效的 AppLocker 策略。" -ForegroundColor Yellow
    }

    # Emit a ready-to-apply allow helper next to the binary (hash-pinned).
    # Built with [string]::Join so this Go raw template never needs PowerShell
    # backtick-escapes (which would terminate the Go raw string).
    ...
    $q = [char]39
    $AllowBody = [string]::Join([char]10, @(
      '#Requires -RunAsAdministrator',
      '# AIOps: allowlist helper for Application Control. Generated by the installer.',
      '$ErrorActionPreference = ''Continue''',
      ('$Exe = ' + $q + $AgentExe.Replace($q, $q+$q) + $q),
      ('$Sha = ' + $q + $Actual + $q),
      ('$Xml = ' + $q + $AllowXml.Replace($q, $q+$q) + $q),
      ('$Cip = ' + $q + $AllowCip.Replace($q, $q+$q) + $q),
    ...
    try {
      [System.IO.File]::WriteAllText($AllowPs1, $AllowBody, (New-Object System.Text.UTF8Encoding $false))
      Write-Host ""
      Write-Host "[AIOps] Fix options / 处理办法:" -ForegroundColor Yellow
      Write-Host "  A) 管理员 PowerShell 运行放行脚本（推荐先试）:"
      Write-Host ('     powershell -ExecutionPolicy Bypass -File "' + $AllowPs1 + '"')
      Write-Host "  B) Smart App Control: Settings → Privacy & security → Windows Security → App & browser control → Smart App Control → Off"
      Write-Host "     （若开关灰掉无法关闭，只能企业放行或换已签名 Agent）"
      Write-Host "  C) IT / GPO 放行 SHA-256:"
      Write-Host ("     " + $Actual)
      Write-Host ("     Path: " + $AgentExe)
      Write-Host ("  D) Verify hash matches server: $Server/dl/aiops-agent.exe.sha256")
    } catch {
      Write-Host "[AIOps] Fix options / 处理办法:" -ForegroundColor Yellow
      Write-Host "  1) WDAC/AppLocker: allow Publisher/Path/Hash for $AgentExe (IT / GPO)."
      Write-Host "  2) Consumer Smart App Control → Off, then retry."
      Write-Host ("  3) local SHA-256 = " + $Actual)
    }
  }
